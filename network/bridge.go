...
func (d *BridgeNetworkDriver) Connect(network *Network, endpoint *Endpoint) error {
	...
	if err = netlink.LinkAdd(&endpoint.Device); err != nil {
		logrus.Errorf("add endpoinnt device, err: %v", err)
		return err
	}
...
func setInterfaceIP(name string, rawIP string) error {
	...
	if err != nil {
		logrus.Debugf("abandoning retrieving new bridge link from netlink, Run [ip link] to troubleshoot the error: %v", err)
		return err
	}
